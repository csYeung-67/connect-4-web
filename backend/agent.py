"""
四子棋 AI Agent - 從 main.py 提取並適配為可被 API 調用的模組
"""
import numpy as np
import random
...
def get_ai_move(board, mark, rows=6, columns=7, inarow=4, depth=3):
    """
    獲取 AI 的下一步移動（API 調用接口）
    
    參數:
        board: 一維數組，表示棋盤狀態（扁平化）
        mark: AI 的玩家標記 (1 或 2)
        rows: 棋盤行數，默認 6
        columns: 棋盤列數，默認 7
        inarow: 連子數，默認 4
        depth: minimax 搜尋深度，默認 3
    
    返回:
        int: AI 選擇的列索引 (0 到 columns-1)
    """
    # 創建配置對象
    config = Config(rows=rows, columns=columns, inarow=inarow)
    
    # 轉換盤面格式：將一維數組重塑為二維棋盤
    grid = np.asarray(board).reshape(rows, columns)
    
    # 執行 Minimax 演算法
    score, best_move = minimax(grid, depth=depth, is_maximizing=True, mark=mark, config=config)
    
    # 安全檢查：如果 minimax 返回 None，使用隨機選擇作為備選
